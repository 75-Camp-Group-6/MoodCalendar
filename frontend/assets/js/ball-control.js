...
class Point extends HSLColor {
	constructor(x, y, clientSize) {
		...
		this._x = x
		this._y = this.yJudge(y)
		this.getPoint = this.getPoint.bind(this)
		this.setPoint = this.setPoint.bind(this)
		this.setHSLColor = this.setHSLColor.bind(this)
		this.yJudge = this.yJudge.bind(this)
		// 有效点设置对应颜色
	...
	setHSLColor (...args) {
		if (args.length) {
			...
			let x = (this._x - this._cW * 0.5) / this._cW * 2,  // [-1, 1]
				y = (this._y - this._cH * 0.3) / this._cH * 10 / 3  // [-1, 1]
			// 转换极坐标系
	...
	yJudge(y) {
		return y > 0.6 * this._cH ? (0.6 * this._cH) : y
	}
}
...
let initBall = ($screen, $ball, ...args) => {
	let clientSize = {
		cW: $screen.documentElement.clientWidth,
		cH: $screen.documentElement.clientHeight
	}
...
window.onload = function () {
	let ball = initBall(document, document.getElementById("moodBall"))
